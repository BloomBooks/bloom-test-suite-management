...
// Like richText(), this returns the *fragment array*, not a property value, so
// callers wrap it themselves: `{ "Test Case Run": { title: titleText(x) } }`.
// Passing the bare array as the property value is the easy mistake, and Notion
// reports the resulting 400 against some *other* property in the same PATCH
// ("body.properties.<other>.start should be defined"), so the error text points
// nowhere near the real problem. If a property write fails for no apparent
// reason, check every property in the payload for a missing type wrapper.
