...
- `area-mapping.json`, `title-mapping.json`, `step-overrides.json`, `curation.json`
  Curation inputs read by `prepare-import.mjs` to interpret the spreadsheet: `area-mapping.json` classifies context rows (area / instruction / ignore) and supplies area names and prepended instructions; `title-mapping.json` and built-in title overrides produce clean case titles; `step-overrides.json` replaces the auto-derived checklist steps / notes for specific cases by `Import ID`; `curation.json` holds per-row manual decisions keyed by original spreadsheet row number — `instructionRows` are dropped as test cases and their text is prepended to the following tests, and `ignorePriorityRows` are forced to a priority of `Ignore` (used for rows whose source priority cell is blank but should be ignored). Row numbers are stable because the source spreadsheet is frozen.
